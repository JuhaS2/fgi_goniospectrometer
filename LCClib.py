...
@author: jouni
"""
import visa  # pip3 install pyvisa pyserial pyusb
import time
import numpy as np
...
LCC=None
resources = visa.ResourceManager('@py')
trialres=['ASRL/dev/ttyUSB0::INSTR','ASRL/dev/ttyUSB1::INSTR','ASRL/dev/ttyUSB3::INSTR','ASRL/dev/ttyUSB4::INSTR','ASRL/dev/ttyUSB5::INSTR','ASRL/dev/ttyUSB6::INSTR','ASRL/dev/ttyUSB7::INSTR','ASRL/dev/ttyUSB8::INSTR']
for ress in trialres:
    try:
        ...
        continue
#    try:
#        LCC = resources.open_resource('ASRL/dev/ttyUSB4::INSTR')  # select the right device
#    except:  
#        try:
#            LCC = resources.open_resource('ASRL/dev/ttyUSB5::INSTR')  # select the right device
#        except: 
#            LCC = resources.open_resource('ASRL/dev/ttyUSB6::INSTR')  # select the right device
if (LCC==None):
    ...
    NRets=3
    RetStep=103
    retardances=RetStep*np.arange(NRets-1,-1,-1)  # more optimal to move this way
